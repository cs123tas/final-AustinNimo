...
//    for (int i = 0; i < vertices.size(); i ++) {//Loop through each y vertex in order to assign a color based on height/elevation
//        //for (int j = 0; j < biomeColors.size(); j++) {
//            if (vertices[i].y <= (max*.80)) {
...
//    }
    if(position.y<.9){
        color = biomeColors[0].color;
        color = glm::vec3(101/255.0, 67/255.0, 33/255.0);
    }
    if(position.y>.9){
//        color = biomeColors[1].color;
        color= glm::vec3(255/255.0, 255/255.0, 255/255.0);
    }
    if(col>50 &&row>50){
        color = glm::vec3(132/255.0, 192/255.0, 17/255.0);
    }
    if(col>0 &&col>40 &&row<20){
        color = glm::vec3(156/255.0, 211/255.0, 219/255.0);
    }
    //colors.push_back(color);
    return color;
}
...
void Terrain::init(GLuint m_program) {
    ...
    std::vector<glm::vec3> data(9 * numVertices);
    int index = 0;
    int offset = 0;
     for(int i =0; i< 3; i++){
    for (int row = 0+offset; row < m_numRows - 1+offset; row++) {
        for (int col = m_numCols - 1+offset; col >= 0+offset; col--) {
            data[index++] = getPosition(row, col);
            data[index++] = getNormal  (row, col);
            data[index++] = generate_biome(getPosition(row, col), row, col);
            data[index++] = getPosition(row + 1, col);
            data[index++] = getNormal  (row + 1, col);
            data[index++] = generate_biome(getPosition(row+1, col),row + 1, col);
        }
        data[index++] = getPosition(row + 1, 0);
        data[index++] = getNormal  (row + 1, 0);
        data[index++] = generate_biome(getPosition(row+1, 0),row + 1, 0);
        data[index++] = getPosition(row + 1, m_numCols - 1);
        data[index++] = getNormal  (row + 1, m_numCols - 1);
        data[index++] = generate_biome(getPosition(row+1, m_numCols - 1),row + 1, m_numCols - 1);
   ...
   // glUniform3f(glGetUniformLocation(m_program, "color"), 0, 1.0f, 0);

    // Vertex Buffer Object
//    std::vector<VBOAttribMarker> marker;
//    marker.push_back(VBOAttribMarker(ShaderAttrib::COLOR, 3, 0, VBOAttribMarker::DATA_TYPE::FLOAT, false));
//    CS123::GL::VBO vbo = VBO(&colors[0][0], colors.size() * 3, marker,VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP);
//    std::unique_ptr<VAO> vao = std::make_unique<VAO>(vbo, numVertices);

    m_shape->setAttribute(ShaderAttrib::COLOR, 3, sizeof(glm::vec3)*2, VBOAttribMarker::DATA_TYPE::FLOAT, false);
//    glUniform3f(glGetUniformLocation(m_program,"color"),colors);
   // m_shape->setAttribute(ShaderAttrib::COLOR, 3colors.size() * sizeof(float), 0,VBOAttribMarker::DATA_TYPE::FLOAT, GL_STATIC_DRAW);
